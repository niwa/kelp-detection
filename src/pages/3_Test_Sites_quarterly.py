...
def get_map(kelp_total_extents: geopandas.GeoDataFrame, land: geopandas.GeoDataFrame, kelp_info: pandas.DataFrame, display_range: list):
    
    if streamlit.session_state.quarterly_index_3 != streamlit.session_state.quareterly_map_index_3 or streamlit.session_state.quareterly_percentiles_3 != display_range:
        ...
        for index, tile_id in enumerate(tile_ids):
            folium_map.add_stac_layer(collection=collection, item=tile_id, assets=rgb_bands, name="RGB",
                                      titiler_endpoint="pc", rescale=f"{display_range[index][0]},{display_range[index][1]}", fit_bounds=False)
...
def main():
    ...
    if 'quarterly_index_3' not in streamlit.session_state:
        ...
        streamlit.session_state.quareterly_percentiles_3 = None
    ...
    if len(streamlit.session_state.quarterly_index_3):
        date_index = streamlit.session_state.quarterly_index_3[0]
        ...
        folium_map = get_map(kelp_total_extents, land, kelp_info, display_range)
